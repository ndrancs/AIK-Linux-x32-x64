...
clear;
echo "\nAndroid Image Kitchen - RepackImg Script - x64";
...
case $args in
  ...
  *)
    ...
    case $ramdiskcomp in
      ...
      bzip2) compext=bz2;;
      lz4) repackcmd="$bin/lz4_x64 -l stdin stdout";;
    esac;
    cd ramdisk;
...
echo "\nBuilding image...\n"
$bin/mkbootimg_x64 --kernel "split_img/$kernel" --ramdisk "$ramdisk" $second --cmdline "$cmdline" --board "$board" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff $secondoff --tags_offset $tagsoff $dtb -o image-new.img;
if [ $? -eq "1" ]; then
  abort;
  return 1;
fi;
